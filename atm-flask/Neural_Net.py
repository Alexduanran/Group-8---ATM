...
class NeuralNetwork():
    ...
    def __init__(self, durnkness_index, structure=[3,1]):
        self.structure = structure
        self.durnkness_index = durnkness_index
    ...
    def randomize_synaptic_weights(self):
        for layer in range(self.depth - 1):
            self.synaptic_weights[layer] = 2 * np.random.random((self.structure[layer], self.structure[layer + 1])) - 1
        return 0
        ...
        for starting_point_i in range(num_of_starting_points):
            ...
            for iteration in range(training_iterations):
                ...
                for i in range(0, self.depth - 1):
                    self.synaptic_weights[i] += layer[i].T.dot(delta[i + 1]) * self.durnkness_index
